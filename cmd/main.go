...
func parseG(fileName string) (Graph, int, string, string) {
	...
	// guard against possible trailing CR on first line
	...
	if err != nil {
		...
		// banner removed intentionally to keep exact required output format
	}
	start, end := "", ""
	flag := "room"
	for _, line := range lines[1:] {
...
func main() {
	...
	// Echo file: print first line (number of ants) and then the rest of the file (without duplicating the first line)
	dataRaw, err := os.ReadFile(fileName)
	if err == nil {
		dataStr := strings.ReplaceAll(string(dataRaw), "\r\n", "\n")
		lines := strings.Split(dataStr, "\n")
		if len(lines) > 0 && lines[0] != "" {
			fmt.Println(lines[0]) // print only the first line (number of ants)
		}
		if len(lines) > 1 {
			// print rest of file (from second line), preserving original formatting
	...
	for height > 1 {
		...
		if len(structAnt) > 0 {
			// visualization removed; structAnt kept for algorithm only
		}
